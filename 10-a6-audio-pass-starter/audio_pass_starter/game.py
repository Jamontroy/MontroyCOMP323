...
class TitleScene:
    ...
    def on_enter(self) -> None:
        self.game.audio.play_loop("menu_music")
...
class EndScene:
    ...
    def on_enter(self) -> None:
        ...
        if self.won:        #Added
            self.game.audio.play("victory_sound")
        else:
            self.game.audio.play("gameover_sound")
...
class PlayScene:
    ...
    def __init__(self, game: Game) -> None:
        ...
        self.score = 0
        self.banner = "Scan is on cooldown."
        self.banner_for = 2.0
        self.scan_hint_for = 0.0

    def on_enter(self) -> None:
        self.game.audio.play_loop("game_music") #Added
    ...
    def update(self, dt: float) -> None:
        ...
        for node in self.nodes:
            if self._overlap(self.player.pos, self.player.radius, node.pos, node.radius):
                ...
                self.banner = "Node collected!"
                self.game.audio.play("collect_node")
        ...
        for hazard in self.hazards:
            ...
            if self._overlap(self.player.pos, self.player.radius, hazard.pos, hazard.radius):
                ...
                self.banner = "Watch Out!" #Added
                self.game.audio.play("hazard_collide")
                self.banner_for = 0.9
    ...
    def _draw_hud(self, screen: pygame.Surface) -> None:
        ...
        todo = self.banner if self.banner_for > 0.0 else ""
