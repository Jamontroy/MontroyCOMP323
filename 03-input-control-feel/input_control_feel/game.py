...
class Game:
    ...
    def __init__(self) -> None:
        ...
        #Added
        self.double_jump_requested = False
    ...
    def _reset(self, keep_state: bool = False) -> None:
        ...
        self.on_ground = True
        self.jump_requested = False
        self.double_jump_requested = False #Added
        self.double_jump_used = False #Added
        self.dash_jump_cooldown = 0.0
        self.last_move_dir = pygame.Vector2(1, 0)
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        #Added    
        if self.platformer_mode and self.jump_requested and event.key in {pygame.K_UP, pygame.K_w, pygame.K_SPACE}:
            self.double_jump_requested = True
    ...
    def _read_direction(self) -> pygame.Vector2:
        ...
        if any(keys[k] for k in mapping[Action.MOVE_LEFT]):
            x -= 1
        if any(keys[k] for k in mapping[Action.MOVE_RIGHT]):
            x += 1
        if any(keys[k] for k in mapping[Action.MOVE_UP]):
            y -= 1
        if any(keys[k] for k in mapping[Action.MOVE_DOWN]):
            y += 1
    ...
    def draw(self) -> None:
        ...
        # Added Player color feedback
        if self.double_jump_used:
            color = (191, 97, 106)
        else:
            color = (94, 129, 172)

        pygame.draw.rect(self.screen, color, self.player_rect, border_radius=6)
