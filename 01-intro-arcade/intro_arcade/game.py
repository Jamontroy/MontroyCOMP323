...
class Game:
    ...
    def _reset_run(self) -> None:
        ...
        self.score = 0
        self.alive_time = 0.0
        self.bar = 5 #ADDED PORTION
        self.speed = 360.0 #ADDED PORTION
        self.enemySpeedup = 1 #ADDED PORTION
    ...
    def update(self, dt: float) -> None:
        ...
        self.player_v.x = input_x * self.speed #added self to speed
        self.player_v.y = input_y * self.speed #added self to speed
        ...
        for i, r in enumerate(self.enemy_rects):
            v = self.enemy_vs[i]
            r.x += int(v.x * self.enemySpeedup * dt)
            r.y += int(v.y * self.enemySpeedup * dt)
        ...
        if self.player.colliderect(self.coin):
            self.score += 1
            self.coin = self._spawn_coin()
            self.speed += 18 #Why 18? Since the original speed is set to 360 and I wanted it to essentially double every 20 coins 360/20 = 18. This multiplies the self.speed variable that I added/changed from the local "speed"
            self.enemySpeedup += 0.05 #Similarly to self.speed I wanted it to double for every 20 coins, thus 0.5. This adds a multiplier to the vector calculation with the enemyRects

        # I figured I would leave this in and have it commented out... It does a similar function just with more steps. Originally I was going to have the speed raise every 5 coins to the point it would double at 20, but I decided it is smoother game design to increment every coin collected instead of every 5, so I moved the function into the "collide with coin" method.

        # if self.score >= self.bar:
        #     self.bar += 5 #Iterates to the next amount the speed will be raised at.
        #     self.speed += 90 #Multiplies the self.speed variable that I added/changed from the local "speed"
        #     self.enemySpeedup += 0.25 #Adds a multiplier to the vector calculation with the enemyRects
