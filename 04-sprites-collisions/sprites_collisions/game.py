...
class Palette:
    ...
    key: pygame.Color = field(default_factory=lambda: pygame.Color("#a3be8c"))
...
class Coin(pygame.sprite.Sprite):
    def __init__(
        ...
        *,
        # hitbox_size: int = 30, Removed because its not nescessary as I can use the visual_size with the correct calculation.
        visual_size: int = 30,
        color: pygame.Color,
    ) -> None:
        super().__init__()
        self.rect = pygame.Rect(0, 0, visual_size, visual_size)
        self.rect.center = center
        self.radius = (visual_size * 0.9 / 2) # Added based on the document in sakai
...
class Unlock_Key(pygame.sprite.Sprite):
    def __init__(
        self,
        center: tuple[int, int],
        *,
        ...
        self.visual_size = size
        self.home = pygame.Vector2(center)
        self.speed = speed
        self.radius = (size * 0.9 / 2)
...
class Player(pygame.sprite.Sprite):
    def __init__(
        ...
        center: tuple[int, int],
        *,
        # hitbox_size: int = 35, Removed because its not nescessary as I can use the visual_size with the correct calculation.
        visual_size: int = 38,
        color: pygame.Color,
    ) -> None:
        super().__init__()
...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self.unlock_keys: pygame.sprite.Group[Unlock_Key] = pygame.sprite.Group()
    ...
    def _reset_level(self, *, keep_state: bool = False) -> None:
        ...
        #Unlock key (trigger) ADDED
        k1 = Unlock_Key((self.playfield.centerx + 180, self.playfield.centery + 80), color=self.palette.key)
        self.unlock_keys.add(k1)
        self.all_sprites.add(k1)

        # Hazards (damage)
        h1 = Hazard((self.playfield.centerx + 180, self.playfield.centery - 80), color=self.palette.hazard)
        h2 = Hazard((self.playfield.centerx - 140, self.playfield.centery + 140),color=self.palette.hazard,patrol_dx=110,speed=220.0,)
        h3 = Hazard((self.playfield.centerx + 100, self.playfield.centery + 110),color=self.palette.hazard,patrol_dx=110,speed=160.0,)
    ...
    def update(self, dt: float) -> None:
        ...
        if self.state not in {"play_noKey", "play_key"}:
            return
        ...
        picked = pygame.sprite.spritecollide(self.player, self.coins, dokill=True, collided=pygame.sprite.collide_circle)
        if picked:
            self.player.score += len(picked)

        # Hazards: damage + response
        for hz in pygame.sprite.spritecollide(self.player, self.hazards, dokill=False, collided=pygame.sprite.collide_circle):
            if(self.state == "play_noKey"):
                self._apply_damage(hz.rect)
            elif(self.state == "play_key"):
                hz.kill()

        self.hazards.update(dt)

        # Unlock keys:
        for key in pygame.sprite.spritecollide(self.player, self.unlock_keys, dokill=True, collided=pygame.sprite.collide_circle):
            self.state = "play_key"
            for hz in self.hazards:
                hz.color = pygame.Color("#a3be8c")
    ...
    def draw(self) -> None:
        ...
        # Draw unlock keys
        for key in self.unlock_keys: #outline
            visual = pygame.Rect(0, 0, key.visual_size, key.visual_size)
            visual.center = key.rect.center
            pygame.draw.polygon(self.screen, key.color, [
                (visual.centerx, visual.top),
                (visual.right, visual.centery),
                (visual.centerx, visual.bottom),
                (visual.left, visual.centery)
            ], 0)
            #inside
            pygame.draw.polygon(self.screen, pygame.Color("#000000"), [
                (visual.centerx, visual.top),
                (visual.right, visual.centery),
                (visual.centerx, visual.bottom),
                (visual.left, visual.centery)
        ...
        for hazard in self.hazards:
            ...
            pygame.draw.rect(self.screen, hazard.color, r)                      #Change hazards to be rects
            pygame.draw.rect(self.screen, pygame.Color("#000000"), r, 2)
    ...
    def _draw_debug(self, cam: pygame.Vector2) -> None:
        # Hitboxes
        pygame.draw.circle(self.screen, pygame.Color("#FF0000"), self.player.rect.center + cam, self.player.radius, 2)
        for coin in self.coins:
            pygame.draw.circle(self.screen, pygame.Color("#FF0000"), coin.rect.center + cam, coin.radius, 2)
        for hazard in self.hazards:
            pygame.draw.rect(self.screen, pygame.Color("#0000FF"), hazard.rect.move(cam), 2)
        for key in self.unlock_keys:
            pygame.draw.circle(self.screen, pygame.Color("#ebcb8b"), key.rect.center + cam, key.radius, 2)
